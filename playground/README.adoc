...
*Scaffold.* The directory structure is in place. The interactive
playground itself is not yet built.
...
[arabic]
. Write a minimal Deno server that serves `+public/+` + a `+/run+`
endpoint
. Build tangle-wasm to WASM with `+wasm-pack+` or similar
. Add Monaco editor with Tangle syntax highlighting
. Add execution-mode tabs
. Add example loader wiring up `+playground/examples/+`
