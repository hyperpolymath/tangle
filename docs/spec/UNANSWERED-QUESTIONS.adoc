...
[width="100%",cols="10%,29%,29%,32%",options="header",]
|===
...
|A4.4 |Assertion decidability |Runtime only MVP; future: assert vs prove
|D1.15.1
...
|B3.1 |Module re-exports |Private for MVP; noted for future review
|D2.11
|===

==== Future Review Items

These were explicitly flagged for post-MVP reconsideration: 1. *A5.1 /
D1.15*: Richer error handling model (exceptions, Result types) 2. *A4.4
/ D1.15.1*: `+assert+` vs `+prove+` split for static verification 3.
*B3.1 / D2.11*: Module re-exports (Rust-style `+pub use+`)
